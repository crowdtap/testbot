...
module Testbot::Server
  ...
  get '/jobs/next' do
    ...
    if next_job
      ...
      [ next_job.id, next_job.requester_mac, next_job.project, next_job.root, next_job.type, (next_job.jruby == 1 ? 'jruby' : 'ruby'), next_job.files, next_job.git_hash, next_job.git_repo, next_job.build.id ].join(',')
    end
  end
