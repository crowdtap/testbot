module Testbot::Server
  ...
  class Job < MemoryModel
    ...
    def self.next(params, remove_addr)
      clean_params = params.reject { |k, v| %w(build_id requester_mac no_jruby).include?(k) }
      ...
      [ next_job(params["build_id"], params["requester_mac"], params["no_jruby"]), runner ]
    end

    private

    def self.next_job(build_id, requester_mac, no_jruby)
